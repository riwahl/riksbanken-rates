import psycopg2
from fetch_data import fetch_latest_data_for_series, fetch_data_for_series_for_range
...
def create_tables(conn: psycopg2.extensions.connection, series_ids: dict[str, any]) -> None:
    ...
    try:
        cursor = conn.cursor()
        for table_name in series_ids.keys():
            cursor.execute(f"""
...
def insert_records(conn, series_ids: dict[str, any], data_dict: dict[str, any]) -> None:
    try:
        ...
        for table_name, series_name in series_ids.items():
            data = data_dict.get(table_name)
            # If data is a dict, wrap it in a list for uniform processing
            ...
            for row in data:
                ...
                print(f"Record inserted into dev.{table_name} for date {row['date']}.")
            conn.commit()
    except psycopg2.Error as e:
        print(f"Error inserting records: {e}")
        raise

def fetch_existing_records(conn, table: str) -> dict[str, any]:
    cursor = conn.cursor()
    cursor.execute(f"SELECT date::DATE, value FROM dev.{table};")
    records = cursor.fetchall()
    return {record[0]: record[1] for record in records}

def main():
    try:
        data_dict = {series_name: fetch_data_for_series_for_range(series_id) for series_name, series_id in series_ids.items()}
        conn = connect_to_db()
